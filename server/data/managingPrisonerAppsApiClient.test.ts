...
describe('ManagingPrisonerAppsApiClient', () => {
  ...
  beforeEach(() => {
    mockAuthenticationClientInstance = {
      getToken: jest.fn().mockResolvedValue(user.token),
    } as unknown as jest.Mocked<AuthenticationClient>

    fakeManagingPrisonerAppApi = nock(config.apis.managingPrisonerApps.url)
    client = new ManagingPrisonerAppsApiClient(mockAuthenticationClientInstance)
  })
  ...
  it('should retrieve a specific prisoner application by prisoner and application ID', async () => {
    ...
    const output = await client.getPrisonerApp(user.username, 'prisoner-id', 'app-id')
  ...
  it('should submit a new prisoner application', async () => {
    ...
    const output = await client.submitPrisonerApp(user.username, submitPrisonerAppData)
    expect(output).toEqual(app)
  })
  ...
  it('should search for applications based on payload', async () => {
    ...
    const output = await client.getApps(user.username, appSearchPayload)
  ...
  it('should add a response to an application', async () => {
    ...
    const output = await client.addResponse(user.username, 'prisoner-id', 'app-id', {
      reason: '',
      decision: 'APPROVED',
      appliesTo: ['id'],
    })
  ...
  it('should fetch the response for an application', async () => {
    ...
    const output = await client.getResponse(user.username, 'prisoner-id', 'app-id', 'response-id')
  ...
  it('should update the form data for an application', async () => {
    ...
    const output = await client.changeApp(user.username, 'prisoner-id', 'app-id', payload)
  ...
  it('should fetch the history for an application', async () => {
    ...
    const output = await client.getHistory(user.username, 'prisoner-id', 'app-id')
  ...
  it('should retrieve a prisoner application with photos', async () => {
    ...
    const output = await client.getPrisonerApp(user.username, 'G4567', '23d2c453-be11-44a8-9861-21fd8ae6e922')
  ...
  it('should handle the complete photo submission and retrieval flow', async () => {
    ...
    const submittedApp = await client.submitPrisonerApp(user.username, submitPrisonerAppDataWithPhotos)
    ...
    const retrievedApp = await client.getPrisonerApp(user.username, prisonerId, appId)
  ...
  it('should submit application without photos when fileRequestDtos is empty', async () => {
    ...
    const output = await client.submitPrisonerApp(user.username, appDataNoPhotos)
  ...
  it('should handle errors when submitting application with photos fails', async () => {
    ...
    const result = await client.submitPrisonerApp(user.username, submitPrisonerAppDataWithPhotos)
  ...
  it('should handle application with empty files array', async () => {
    ...
    const output = await client.getPrisonerApp(user.username, 'G4567', 'app-id')
  ...
  it('should handle 404 when application with photos does not exist', async () => {
    ...
    const result = await client.getPrisonerApp(user.username, 'G4567', 'non-existent-id')
    expect(result).toBeNull()
  })
})
